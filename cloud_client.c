...
static size_t parse_response(const char *json, uint32_t *received_ids, size_t received_cap)
{
    ...
    if (cJSON_IsArray(cmds) && g_cfg.command_cb) {
        ...
        cJSON_ArrayForEach(c, cmds) {
            ...
            if (relay >= 1 && relay <= 5 && (state == 0 || state == 1)) {
                uint32_t command_id = (uint32_t)idv->valuedouble;
                bool accepted = g_cfg.command_cb(relay - 1, state, g_cfg.ctx);
                if (accepted) {
                    add_pending_ack(command_id);
                    if (received_count < received_cap) received_ids[received_count++] = command_id;
                }
            }
        }
    }
...
static void remove_acks_sent_successfully(const uint32_t *sent_ids, size_t sent_count,
                                          const uint32_t *received_ids, size_t received_count)
{
    if (!sent_ids || sent_count == 0) return;
    size_t w = 0;
    for (size_t i = 0; i < pending_ack_count; ++i) {
        bool sent = false, reissued = false;
        for (size_t j = 0; j < sent_count; ++j) if (pending_ack_ids[i] == sent_ids[j]) { sent = true; break; }
        for (size_t j = 0; j < received_count; ++j) if (pending_ack_ids[i] == received_ids[j]) { reissued = true; break; }
        if (!sent || reissued) pending_ack_ids[w++] = pending_ack_ids[i];
    }
    pending_ack_count = w;
}
...
{
    (void)arg; uint32_t backoff = 0;
    char *body = malloc(4096), *response = malloc(RESPONSE_MAX);
    uint32_t *sent_ack_ids = malloc(sizeof(uint32_t) * COMMAND_ACK_MAX);
    uint32_t *received_ids = malloc(sizeof(uint32_t) * COMMAND_ACK_MAX);
    if (!body || !response || !sent_ack_ids || !received_ids) {
        free(body); free(response); free(sent_ack_ids); free(received_ids);
        ESP_LOGE(TAG, "Cloud buffers allocation failed"); g_task_handle=NULL; g_task_started=false; vTaskDelete(NULL); return;
    }
    esp_task_wdt_add(NULL);
    while (1) {
        if (g_user_offline || g_ota_busy) { g_online=false; vTaskDelay(pdMS_TO_TICKS(500)); esp_task_wdt_reset(); continue; }
        cloud_client_config_t cfg; copy_config(&cfg);
        if (!cfg.base_url[0] || !cfg.device_id[0] || !cfg.device_token[0]) { g_online=false; vTaskDelay(pdMS_TO_TICKS(2000)); esp_task_wdt_reset(); continue; }
        int states[5]={0}; bool enabled[5]={0}; if (cfg.snapshot_cb) cfg.snapshot_cb(states, enabled, cfg.ctx);
        size_t sent_ack_count = pending_ack_count < COMMAND_ACK_MAX ? pending_ack_count : COMMAND_ACK_MAX;
        memcpy(sent_ack_ids, pending_ack_ids, sent_ack_count*sizeof(uint32_t));
        bool body_ok = build_poll_body(&cfg, body, 4096, states, enabled) > 0;
        bool ok = body_ok && cloud_post(&cfg, "/api/device/poll", body, response, RESPONSE_MAX); g_online=ok;
        if (ok) { size_t received_count=parse_response(response, received_ids, COMMAND_ACK_MAX); remove_acks_sent_successfully(sent_ack_ids,sent_ack_count,received_ids,received_count); backoff=0; }
        else { if (backoff==0) backoff=5; else if (backoff<MAX_BACKOFF_SECONDS) backoff=(backoff*2>MAX_BACKOFF_SECONDS)?MAX_BACKOFF_SECONDS:backoff*2; }
        uint32_t delay=ok?POLL_SECONDS:backoff;
        for(uint32_t sec=0;sec<delay;++sec){ if(g_task_handle && ulTaskNotifyTake(pdTRUE,pdMS_TO_TICKS(1000))>0) break; esp_task_wdt_reset(); if(g_user_offline||g_ota_busy) break; }
    }
}
...
void cloud_client_init(const cloud_client_config_t *cfg)
{
    ...
    if (xTaskCreate(cloud_task, "cloud_client", 10240, NULL, 3, &g_task_handle) == pdPASS) {
        g_task_started = true;
    } else {
...
void cloud_client_set_ota_busy(bool busy)
{
    g_ota_busy = busy;
    if (busy) g_online = false;
    if (g_task_handle) xTaskNotifyGive(g_task_handle);
}
